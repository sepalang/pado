...
(function(FN,REMOVE_VALUE,HAS_VALUE,REDUCE,SELECT,CLONE,FOR_MAP,RANGE,DOMAIN_RANGE_VALUE){
  ...
  var Block = function(posSize,syncOpt){
    this.$space = (void 0);
    this.$posSize;
    this.$mask;
  ...
  Block.prototype = {
    sync:function(block,syncOpt){ 
      ...
      if(block instanceof Block){ 
        ...
        this.$posSize = FOR_MAP(CLONE(block),function(posSize){ return !_.isArray(posSize) ? [posSize, 0] : posSize; });
      }
      return this; 
    },
    ...
    get:function(){ return CLONE(typeof this.$posSize === "function" ? this.$posSize() : this.$posSize); },
    domainValue:function(){ return FOR_MAP(CLONE(this.get()),function(posSize){return posSize[0];}); },
    domainSize :function(){ return FOR_MAP(CLONE(this.get()),function(posSize){return posSize[1];}); },
    domainMap:function(){
    ...
    isOverflow:function(mask){
      var overflow = false;
      FOR_MAP(this.overflow(mask),function(f){ if(f){ overflow = true; } });
      return overflow;
    },
    ...
    rangeStart:function(){ return this.$space.domainRange(FOR_MAP(this.get(),function(posSize){ return posSize[0]; })); },
    rangeSize:function(){ return this.$space.domainRangeSize(FOR_MAP(this.get(),function(posSize){ return posSize[1]; })); },
    rangeMap:function(){
  ...
  var Tracker = function(space,domainMask){
    this.$space = space;
  ...
  Tracker.prototype = {
    ...
    domainBlock:function(cursor,callback){
      var domainGrid = FOR_MAP(this.$space.getRange(),function(range){ return range[2]; });
      var block      = this.block(FOR_MAP(this.$space.rangeDomain(cursor),function(cursorPoint,key){ return [cursorPoint,SELECT(domainGrid,key)]; }));
  ...
  Space.prototype = {
    domain:function(domain){
      //default grid scale
      domain = FOR_MAP(domain,function(domain){
    ...
    domainRange:function(vs){ return DOMAIN_RANGE_VALUE(this.getDomain(),this.getRange(),vs,this.$niceRange); },
    rangeDomain:function(vs){ return DOMAIN_RANGE_VALUE(this.getRange(),this.getDomain(),vs,this.$niceDomain); },
